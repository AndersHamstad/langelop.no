...
export default function Shop() {
        ...
        <div className="rounded-3xl bg-white p-6 md:p-10">
          <div className="grid gap-8 lg:grid-cols-[1.1fr_0.8fr] lg:gap-10">
            ...
            <aside className="lg:sticky lg:top-24">
              <p className="text-s font-bold uppercase tracking-widest text-gray-900">
                ...
                {status === "success" ? (
                  <div className="mt-5 rounded-2xl border border-green-200 bg-green-50 p-4 text-green-900">
                    ...
                    <p className="mt-1 text-sm">
                      Vi sender deg beskjed når sokken slippes.
                    </p>
                  </div>
                ) : (
                  <form
                  ...
                  >
                    ...
                    >
                      ...
                      <option value="S">S (35–40)</option>
                      <option value="M">M (40–45)</option>
                      <option value="L">L (43–48)</option>
                    </select>
                    ...
                    >
                      {status === "loading" ? "Registrerer…" : "Bli varslet"}
                    </button>

                    <p className="text-xs leading-5 text-gray-500">
                      Ved påmelding godtar du å motta e-post fra Langeløp.no. Du
  ...
  className="shrink-0 p-2 text-gray-600 hover:bg-gray-100 transition"
  aria-label="Lukk"
>
  <X size={18} />
</button>
              ...
              {status === "success" ? (
                ...
                <form onSubmit={onSubmit} className="mt-4 space-y-3">
                  <input
                    ...
                    onChange={(e) => setEmail(e.target.value)}
                    placeholder="E-post"
                    className="w-full rounded-2xl border border-gray-300 bg-white px-4 py-3 text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-green-600"
                  />

                  <select
                    value={sizeInterest}
                    onChange={(e) => setSizeInterest(e.target.value)}
                    className="w-full rounded-2xl border border-gray-300 bg-white px-4 py-3 text-gray-900 focus:outline-none focus:ring-2 focus:ring-green-600"
                  >
                    <option value="">Ønsket størrelse (valgfritt)</option>
                    <option value="S">S (35–40)</option>
                    <option value="M">M (40–45)</option>
                  ...
                  <button
                    type="submit"
                    disabled={status === "loading"}
                    className="w-full rounded-2xl bg-gray-900 py-3.5 font-semibold text-white shadow-sm transition disabled:opacity-60"
                  >
                    {status === "loading" ? "Registrerer…" : "Bli varslet"}
                  </button>
